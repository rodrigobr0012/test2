...
export default function VehicleCreate() {
  ...
  async function handleSubmit(event) {
    ...
    if (!mocksEnabled && !user) {
      setFormError("Faça login para publicar um anúncio.");
      return;
    }

    const normalizedPrice = Number(
      formData.price.replace(/[^0-9,.-]/g, "").replace(",", ".")
    );
    ...
    try {
      ...
      setSubmittedVehicle(created);
      setSuccessMessage(
        mocksEnabled
          ? "Anúncio salvo localmente! Veja abaixo como ele ficará para os compradores."
          : "Veículo cadastrado com sucesso! Confira o resumo antes de compartilhar."
      );
      setFormData({
        ...INITIAL_STATE,
        contactName: user?.full_name || "",
        ...
        >
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            <button
              ...
              className={`${buttonClass} sm:flex-1 disabled:cursor-not-allowed disabled:bg-blue-300`}
            >
              {isSubmitting
                ? "Publicando..."
                : mocksEnabled
